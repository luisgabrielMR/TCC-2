...
O perfil `fixed_200` compara latencia e recursos com alvo de 200 req/s; ele nao representa capacidade maxima. Os perfis `saturation_25` a `saturation_400` usam malha fechada e formam uma bateria separada. Antes deles, a calibracao health-only demonstra a capacidade do instrumento; durante cada rodada, CPU do Locust normalizada pela cota deve ficar abaixo de 90% e a vazao deve permanecer no maximo em 80% da capacidade calibrada. O percentual bruto do cAdvisor tambem e preservado. Saturacao significa apenas o limite pratico observado neste workload e ambiente.
